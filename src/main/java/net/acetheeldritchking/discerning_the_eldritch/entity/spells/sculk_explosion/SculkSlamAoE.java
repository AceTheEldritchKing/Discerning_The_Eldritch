package net.acetheeldritchking.discerning_the_eldritch.entity.spells.sculk_explosion;
...
public class SculkSlamAoE extends AoeEntity {
    public static DamageSource DAMAGE_SOURCE;
    private int waveAnimTime = -1;

    public SculkSlamAoE(EntityType<? extends Projectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
        this.reapplicationDelay = 25;
        this.setCircular();
    }

    public SculkSlamAoE(Level level, float radius) {
        super(DTEEntityRegistry.SCULK_SLAM_AOE.get(), level);
    ...
    public void applyEffect(LivingEntity target) {
        ...
        {
            DAMAGE_SOURCE = new DamageSource(DamageSources.getHolderFromResource(target, DTEDamageTypes.SCULK_CORROSION), this, getOwner());
        }
        ...
        if (waveAnimTime++ < radius)
        {
            if (!level.isClientSide())
            {
                ...
                for (int i = 0; i < maxBlocks; i++)
                {
                    ...
                    if (level.getBlockState(blockPos.below()).isFaceSturdy(level, blockPos.below(), Direction.DOWN))
                    {
                        Utils.createTremorBlockWithState(level, Blocks.SCULK.defaultBlockState(), blockPos, 0.1F + random.nextFloat() * 0.2F);
                    }
                }
            ...
            } else
            {
                ...
                for (int i = 0; i < particles; i++)
                {
                    ...
                    level.addParticle(ParticleTypes.SCULK_SOUL, pos.x, pos.y, pos.z, motion.x, motion.y, motion.z);
